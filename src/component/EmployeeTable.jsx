...
class AdminEmployeeTable extends Component {
  ...
  loadEmployeeData = () => {
    axios
      ...
      .then(async (response) => {
        this.employeeObj = response.data.employee;
        // console.log("response", response.data.employee);

        this.setState({ employeeData: response.data.employee });
        this.setState({ loading: false });

        const rolePromises = this.employeeObj.map((data) =>
          axios.get(process.env.REACT_APP_API_URL + `/api/role/${data.role}`)
        );

        const departmentPromises = this.employeeObj.map((data) =>
          axios.get(
            process.env.REACT_APP_API_URL + `/api/department/${data.department}`
          )
        );

        const positionPromises = this.employeeObj.map((data) =>
          axios.get(
            process.env.REACT_APP_API_URL + `/api/position/${data.position}`
          )
        );
        Promise.all([
          ...rolePromises,
          ...
          const roles = results.slice(0, this.employeeObj.length);
          const departments = results.slice(
            this.employeeObj.length,
            this.employeeObj.length * 2
          );
          const positions = results.slice(this.employeeObj.length * 2);
          // const positions = results.slice(
          //   this.employeeObj.length * 2,
          //   this.employeeObj.length * 3
          // );
          //   const locations = results.slice(this.employeeObj.length * 3);

          this.rowDataT = this.employeeObj.map((data, index) => {
            const role = roles[index].data;
            const department = departments[index].data;
            const position = positions[index].data;
            return {
              data,
              Email: data["Email"],
              ...
              DepartmentName: department
                ...
                : "",
              PositionName: position ? position.position["PositionName"] : "",
